...
interface Lesson {
  ...
  xp_reward: number;
  is_completed: number; // 0 vagy 1 (PHP-ból jön)
}
...
export const SchoolPage = () => {
  ...
  const fetchLessons = async () => {
    try {
      const response = await axios.get(`http://localhost/pawpatrol/api/school/get_lessons.php?user_id=${TEMP_USER_ID}`);
  ...
  const handleComplete = async (lesson: Lesson) => {
    try {
      ...
      if (response.data.success) {
        // 1. Lokálisan frissítjük a listát (zöld pipa)
        setLessons(prev => prev.map(l => l.id === lesson.id ? { ...l, is_completed: 1 } : l));
        
        // 2. Naplózzuk az eseményt a Feedbe ("logolás")
        await logActivity(TEMP_USER_ID, 'learning', `Megtanulta: ${lesson.title} (+${lesson.xp_reward} XP)`);
        
        // 3. Bezárjuk a kártyát
        setExpandedId(null);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {isLoading ? (
          <div className="text-center p-10 text-gray-500">Tananyag betöltése...</div>
      ) : (
